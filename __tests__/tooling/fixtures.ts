...
/** The script the user controller declares: every call below posts to it. */
const userScriptRef: ScriptRef = { kind: 'restlet', scriptId: 'customscript_demo_user', deployId: 'customdeploy_demo_user' };

/** One function per endpoint of the user controller: \`user.api.roles(...)\`. */
export const api = {
    /** The caller and every role assigned to them. Takes no request; the session says who is calling. */
    roles: (options?: ApiCallOptions): Promise<RolesResponse> => callEndpoint<RolesResponse>(userScriptRef, 'roles', {}, options),
};
`;
